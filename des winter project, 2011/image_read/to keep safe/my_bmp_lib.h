...
/************************************************************
         ...
         Retuns a true type image from indexed image
                            ...
                            )                                
{
    ...
    image=indexed_to_true(image);
    
    for(i = 0; i < image->i_header.height; i++){
        for(j = 0; j < image->i_header.width; j++){
            if(image->t_pixel[i][j].r < 100)
                fwrite(&ch1, 1, 1, fp);
...
/***************************************************************
        ...
        Reads a bmp image file (indexed or true color) and 
         returns the bmp_image pointer type variable
                    ...
                    )
{
    ...
    /* Reading image data into ch */
    if(image->f_header.offset != 54) {
        for(i=0; i<image->i_header.height; i++){
            image->i_pixel[image->i_header.height-1-i]=(unsigned char *)malloc(sizeof(char)*new_width);
