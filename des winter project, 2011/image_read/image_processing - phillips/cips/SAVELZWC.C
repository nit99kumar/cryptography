...
is_present(w, k, string_table)
   ...
   struct item string_table[];
{
   int  building, failure, i, last, result, searching;
   char w2[100];
   short a;
...
/* B. */
      if(k == string_table[i].code_char){
         ...
         failure  = 1;
         last     = strlen(w) - 1;
         a        = string_table[i].code_num;
         while(building){
            if(w[last] == string_table[a].code_char){
               last--;
               a = string_table[a].code_num;
               if(last < 0){
                  building  = 0;
                  failure   = 0;
                  searching = 0;
